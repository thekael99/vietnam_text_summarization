...
def train_single_ext(args, device_id):
    ...
    def train_iter_fct():
        return data_loader.Dataloader(args, load_dataset(args, 'train', shuffle=True), args.batch_size, device,True, False, 'ext')
...
def test_text_ext(args):
    ...
    for k in opt.keys():
        if (k in model_flags):
            setattr(args, k, opt[k])
    print(args)
    device = "cpu" if args.visible_gpus == '-1' else "default"
    ...
    trainer = build_trainer(args, device_id, model, None)
    return trainer, device, args
    
def test_inference(trainer,device, args, text_src):
    test_iter = data_loader.load_test_text(args, text_src, args.text_tgt, device)
    return trainer.test(test_iter, -1)
    
def test_text_file_ext(args):
    logger.info('Loading checkpoint from %s' % args.test_from)
    checkpoint = torch.load(args.test_from, map_location=lambda storage, loc: storage)
    opt = vars(checkpoint['opt'])
    for k in opt.keys():
