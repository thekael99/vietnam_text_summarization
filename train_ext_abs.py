...
from models.train_multitask import build_trainer
...
def run(args, device_id, error_queue):
    ...
    try:
        ...
        train_ex_ab_single(args, device_id)
...
def validate_ex_ab(args, device_id, pt, step):
    ...
    tokenizer = BertTokenizer.from_pretrained('NlpHUST/vibert4news-base-cased', cache_dir=args.temp_dir)
...
def test_ex_ab(args, device_id, pt, step):
    device = "cpu" if args.visible_gpus == '-1' else "cuda"
    if (pt != ''):
        test_from = pt
    else:
    ...
    test_iter = data_loader.Dataloader(args, load_dataset(args, 'test', False, 'abs'),
                                       args.test_batch_size, device, False, True, 'abs')
    tokenizer = BertTokenizer.from_pretrained('NlpHUST/vibert4news-base-cased', cache_dir=args.temp_dir)
    format_special_tokens(tokenizer)
    symbols = {'BOS': tokenizer.convert_tokens_to_ids('[unused0]'), 'EOS': tokenizer.convert_tokens_to_ids('[unused1]'),
               'PAD': tokenizer.convert_tokens_to_ids('[PAD]'), 'EOQ': tokenizer.convert_tokens_to_ids('[unused2]')}
...
def test_text_ex_ab(args, device_id, pt, step):
    device = "cpu" if args.visible_gpus == '-1' else "cuda"
    if (pt != ''):
        test_from = pt
    else:
...
def baseline_ext_abs(args, cal_lead=False, cal_oracle=False):
    test_iter = data_loader.Dataloader(args, load_dataset(args, 'test', shuffle=False),
                                       args.batch_size, 'cpu',
                                       shuffle=False, is_test=True)
...
def train_ex_ab(args, device_id):
    if (args.world_size > 1):
        train_ex_ab_multi(args)
    else:
        train_ex_ab_single(args, device_id)


def train_ex_ab_single(args, device_id):
    ...
    def train_iter_fct_ext():
        return data_loader.Dataloader(args, load_dataset(args, 'train', True, 'ext'), args.batch_size, device, True, False, 'ext')
    def train_iter_fct_abs():  
        return data_loader.Dataloader(args, load_dataset(args, 'train', True, 'abs'), args.batch_size, device, True, False, 'abs') 
    ...
    trainer.train(train_iter_fct_ext, train_iter_fct_abs, args.train_steps)