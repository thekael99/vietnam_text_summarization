...
class Batch(object):
    ...
    def __init__(self, task, data=None, device=None, is_test=False):
...
def load_dataset(args, corpus_type, shuffle, task):
    """
    ...
    # Sort the glob output by file name (by increasing indexes).
      print('Loading data for extractive..')
      pts = sorted(glob.glob(args.bert_data_path + '.ext.' + corpus_type + '.[0-9]*.pt'))
    else:
      print('Loading data for abstractive..')
      pts = sorted(glob.glob(args.bert_data_path + '.abs.' + corpus_type + '.[0-9]*.pt'))
    if pts:
        if (shuffle):
            random.shuffle(pts)
        for pt in pts:
        ...
        if task == 'ext':
            pt = args.bert_data_path + '.ext.' + corpus_type + '.0.bert.pt'
        else:
            pt = args.bert_data_path + '.abs.' + corpus_type + '.0.bert.pt'
        yield _lazy_dataset_loader(pt, corpus_type)
...
class Dataloader(object):
    def __init__(self, args, datasets, batch_size,
        ...
        self.task = task
        # self.num_of_data = self._count_data(datasets)
    ...
    def _next_dataset_iterator(self, dataset_iter):
        ...
        return DataIterator(args=self.args,
                            dataset=self.cur_dataset, batch_size=self.batch_size, task = self.task,
                            device=self.device, shuffle=self.shuffle, is_test=self.is_test)


class DataIterator(object):
    def __init__(self, args, dataset, batch_size, task, device=None, is_test=False,
                 shuffle=True):
        self.args = args
    ...
    def __iter__(self):
        while True:
            ...
            for idx, minibatch in enumerate(self.batches):
                ...
                batch = Batch(self.task, minibatch, self.device, self.is_test)
    ...
    if(target_fp == ''):
        ...
        with open(source_fp) as source, open(target_fp) as target:
            for x, y in tqdm(zip(source, target), total=n_lines):
                ...
                batch.batch_size = 1
                yield batch
def load_test_text(args, source_fp, target_fp, device):
    from transformers import BertTokenizer
    tokenizer = BertTokenizer.from_pretrained('NlpHUST/vibert4news-base-cased')
    format_special_tokens(tokenizer)
    sep_vid = tokenizer.convert_tokens_to_ids("[SEP]")
    cls_vid = tokenizer.convert_tokens_to_ids("[CLS]")
    n_lines = len(source_fp.split('\n'))
