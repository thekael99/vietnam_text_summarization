...
def test_text_abs(args, device_id, pt, step):
    ...
    # test_iter = data_loader.Dataloader(args, load_dataset(args, 'test', shuffle=False, task = 'abs'),
    #                                    args.test_batch_size, device, False, True, 'abs')
    tokenizer = BertTokenizer.from_pretrained('NlpHUST/vibert4news-base-cased', cache_dir=args.temp_dir)
    format_special_tokens(tokenizer)
    ...
    symbols = {'BOS': tokenizer.convert_tokens_to_ids('[unused0]'), 'EOS': tokenizer.convert_tokens_to_ids('[unused1]'),
               'PAD': tokenizer.convert_tokens_to_ids('[PAD]'), 'EOQ': tokenizer.convert_tokens_to_ids('[unused2]')}
    predictor = build_predictor(args, tokenizer, symbols, model, logger)
    # test_iter = data_loader.load_text(args, args.text_src, '', "cpu")
    return args, predictor
    
def test_text_abs_reference(args, predictor, text_src):

    test_iter = data_loader.load_test_text(args, text_src, '', "cpu")
    return predictor.translate(test_iter, 0)


def test_text_file_abs(args, device_id, pt, step):
    device = "cpu" if args.visible_gpus == '-1' else "cuda"
    if (pt != ''):
        test_from = pt
    else:
